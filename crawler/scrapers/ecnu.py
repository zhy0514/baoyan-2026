# crawler/scrapers/ecnu.py — 华东师范大学
...
class ECNUScraper(BaseScraper):
    def __init__(self):
        super().__init__()
        self.name = 'ecnu'
        self.university_name = '华东师范大学'
        self.university_short = '华东师大'

    def get_list_page_urls(self) -> list:
        return [
            # 计算机科学与技术学院
            'https://cs.ecnu.edu.cn/',
            # 软件工程学院
            'https://www.sei.ecnu.edu.cn/',
            # 数学科学学院
            'https://math.ecnu.edu.cn/',
            # 研究生院
            'https://yjszs.ecnu.edu.cn/',
            # 统计学院
            'https://stat.ecnu.edu.cn/',
        ]
    ...
    def parse_list_page(self, html: str) -> list:
        ...
        for a in soup.select('a[href]'):
            ...
            if href.startswith('#') or href.startswith('javascript'):
                continue
            full = normalize_url(href, 'https://cs.ecnu.edu.cn')
            if full not in urls:
                urls.append(full)
        return urls
    ...
    def _infer_college(self, url: str, title: str) -> str:
        url_l = url.lower()
        if any(k in url_l for k in ['cs.', 'computer', '计算机']):
            return '计算机科学与技术学院'
        if any(k in url_l for k in ['sei.', 'software', '软件']):
            return '软件工程学院'
        if any(k in url_l for k in ['stat', '统计']):
            return '统计学院'
        if any(k in url_l for k in ['math', '数学']):
            return '数学科学学院'
        if any(k in url_l for k in ['yjszs', '招生', '研究生']):
            return '研究生院'
        return '未知学院'
