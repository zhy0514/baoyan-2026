# crawler/scrapers/fudan.py — 复旦大学
...
class FudanScraper(BaseScraper):
    def __init__(self):
        super().__init__()
        self.name = 'fudan'
        self.university_name = '复旦大学'
        self.university_short = '复旦'
    ...
    def get_list_page_urls(self) -> list:
        return [
            ...
            # 数学科学学院
            'https://math.fudan.edu.cn/',
            # 信息科学与工程学院
            'https://it.fudan.edu.cn/',
        ]
    ...
    def parse_list_page(self, html: str) -> list:
        ...
        for a in soup.select('a[href]'):
            ...
            if href.startswith('#') or href.startswith('javascript'):
                continue
            full = normalize_url(href, 'https://cs.fudan.edu.cn')
            if full not in urls:
                urls.append(full)
        return urls
    ...
    def _infer_college(self, url: str, title: str) -> str:
        ...
        if any(k in url_l for k in ['cs.', 'computer', '计算机']):
            return '计算机科学技术学院'
        if any(k in url_l for k in ['sds', '数据', '大数据']):
            return '大数据学院'
        if any(k in url_l for k in ['math', '数学']):
            return '数学科学学院'
        if any(k in url_l for k in ['it.', '信息科学', '信息工程']):
            return '信息科学与工程学院'
        if any(k in url_l for k in ['gsao', '招生', '研究生']):
            return '研究生院'
        return '未知学院'
