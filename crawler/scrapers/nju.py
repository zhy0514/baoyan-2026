...
class NJUScraper(BaseScraper):
    def __init__(self):
        super().__init__()
        self.name = 'nju'
        self.university_name = '南京大学'
        self.university_short = '南大'
    ...
    def parse_list_page(self, html: str) -> list:
        ...
        for a in soup.select('a[href]'):
            ...
            if href.startswith('#') or href.startswith('javascript'):
                continue
            full = normalize_url(href, 'https://cs.nju.edu.cn')
            if full not in urls:
                urls.append(full)
        return urls
    ...
    def _infer_college(self, url: str, title: str) -> str:
        ...
        if any(k in url_l for k in ['math', '数学']):
            return '数学系'
        if any(k in url_l for k in ['software', '软件']):
            return '软件学院'
