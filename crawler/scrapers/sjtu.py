# crawler/scrapers/sjtu.py — 上海交通大学
...
class SJTUScraper(BaseScraper):
    def __init__(self):
        super().__init__()
        self.name = 'sjtu'
        self.university_name = '上海交通大学'
        self.university_short = '上海交大'
    ...
    def parse_list_page(self, html: str) -> list:
        ...
        for a in soup.select('a[href]'):
            ...
            if href.startswith('#') or href.startswith('javascript'):
                continue
            full = normalize_url(href, 'https://www.sjtu.edu.cn')
            if full not in urls:
                urls.append(full)
        return urls
    ...
    def _infer_college(self, url: str, title: str) -> str:
        url_l = url.lower()
        if any(k in url_l for k in ['seiee', '电院', '电子信息']):
            return '电子信息与电气工程学院'
