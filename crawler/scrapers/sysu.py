...
class SYSUScraper(BaseScraper):
    def __init__(self):
        super().__init__()
        self.name = 'sysu'
        self.university_name = '中山大学'
        self.university_short = '中大'
    ...
    def get_list_page_urls(self) -> list:
        return [
            ...
            # 人工智能学院
            'https://sai.sysu.edu.cn/',
            # 数学学院
            'https://math.sysu.edu.cn/',
            # 研究生院
            'https://graduate.sysu.edu.cn/',
            # 网络空间安全学院
            'https://scst.sysu.edu.cn/',
        ]
    ...
    def parse_list_page(self, html: str) -> list:
        ...
        for a in soup.select('a[href]'):
            ...
            if href.startswith('#') or href.startswith('javascript'):
                continue
            full = normalize_url(href, 'https://cse.sysu.edu.cn')
            if full not in urls:
                urls.append(full)
        return urls
    ...
    def _infer_college(self, url: str, title: str) -> str:
        ...
        if any(k in url_l for k in ['cse.', 'computer', '计算机']):
            return '计算机学院（软件学院）'
        if any(k in url_l for k in ['sai.', '人工智能']):
            return '人工智能学院'
        if any(k in url_l for k in ['math', '数学']):
            return '数学学院'
        if any(k in url_l for k in ['scst.', '网络安全', '网安']):
            return '网络空间安全学院'
        if any(k in url_l for k in ['graduate', '招生', '研究生']):
            return '研究生院'
        return '未知学院'
