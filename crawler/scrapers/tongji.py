# crawler/scrapers/tongji.py — 同济大学
...
class TongjiScraper(BaseScraper):
    def __init__(self):
        super().__init__()
        self.name = 'tongji'
        self.university_name = '同济大学'
        self.university_short = '同济'

    def get_list_page_urls(self) -> list:
        return [
            # 电子与信息工程学院（含计算机系）
            'https://see.tongji.edu.cn/',
            # 软件学院
            'https://sse.tongji.edu.cn/',
            # 数学科学学院
            'https://math.tongji.edu.cn/',
            # 研究生院
    ...
    def parse_list_page(self, html: str) -> list:
        ...
        for a in soup.select('a[href]'):
            ...
            if href.startswith('#') or href.startswith('javascript'):
                continue
            full = normalize_url(href, 'https://see.tongji.edu.cn')
            if full not in urls:
                urls.append(full)
        return urls
    ...
    def _infer_college(self, url: str, title: str) -> str:
        url_l = url.lower()
        if any(k in url_l for k in ['see.', '电信', '电子信息', '电子与信息']):
            return '电子与信息工程学院'
        if any(k in url_l for k in ['sse.', 'software', '软件']):
            return '软件学院'
        if any(k in url_l for k in ['math', '数学']):
            return '数学科学学院'
        if any(k in url_l for k in ['yz.', '招生', '研究生']):
            return '研究生院'
        return '未知学院'
