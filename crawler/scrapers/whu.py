...
class WHUScraper(BaseScraper):
    def __init__(self):
        super().__init__()
        self.name = 'whu'
        self.university_name = '武汉大学'
        self.university_short = '武大'
    ...
    def parse_list_page(self, html: str) -> list:
        ...
        for a in soup.select('a[href]'):
            ...
            if href.startswith('#') or href.startswith('javascript'):
                continue
            full = normalize_url(href, 'https://cs.whu.edu.cn')
            if full not in urls:
                urls.append(full)
        return urls
    ...
    def _infer_college(self, url: str, title: str) -> str:
        url_l = url.lower()
        if any(k in url_l for k in ['cse.', '网络安全', '网安']):
            return '国家网络安全学院'
        if any(k in url_l for k in ['cs.', 'computer', '计算机']):
            return '计算机学院'
        if any(k in url_l for k in ['ai.', '人工智能']):
            return '人工智能学院'
        if any(k in url_l for k in ['math', '数学', '统计']):
            return '数学与统计学院'
        if any(k in url_l for k in ['gs.', '招生', '研究生']):
            return '研究生院'
        return '未知学院'
