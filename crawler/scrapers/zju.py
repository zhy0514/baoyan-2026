...
class ZJUScraper(BaseScraper):
    def __init__(self):
        super().__init__()
        self.name = 'zju'
        self.university_name = '浙江大学'
        self.university_short = '浙大'
    ...
    def parse_list_page(self, html: str) -> list:
        ...
        for a in soup.select('a[href]'):
            ...
            if href.startswith('#') or href.startswith('javascript'):
                continue
            full = normalize_url(href, 'http://www.cs.zju.edu.cn')
            if full not in urls:
                urls.append(full)
        return urls
    ...
    def _infer_college(self, url: str, title: str) -> str:
        ...
        if any(k in url_l for k in ['math', '数学']):
            return '数学科学学院'
        if any(k in url_l for k in ['isee', '信电', '信息与电子']):
            return '信息与电子工程学院'
        if any(k in url_l for k in ['cst.', '软件']):
            return '软件学院'
        if any(k in url_l for k in ['grs', '招生', '研究生']):
            return '研究生院'
        return '未知学院'
